...
def yield_closest_leaf_unit_positions(root_cnode: Node) -> Iterable[Tuple[Unit, Sequence[float]]]:
    """
    Yield the leaf units of the given root cnode together with their positions. Here, the positions are corrected for
    periodic boundary conditions so that they are the closest to the root unit's position.
    ...
    Yields
    ------
    (base.unit.Unit, Sequence[float])
        The leaf unit, the leaf unit's position closest to the root unit's position under consideration of periodic
        boundary conditions.
    """
    if not root_cnode.children:
        yield root_cnode.value, root_cnode.value.position
    else:
        root_position = root_cnode.value.position
        for leaf_node in yield_leaf_nodes(root_cnode):
            shortest_separation = setting.periodic_boundaries.separation_vector(root_position, leaf_node.value.position)
            yield leaf_node.value, [entry + shortest_separation[index] for index, entry in enumerate(root_position)]
