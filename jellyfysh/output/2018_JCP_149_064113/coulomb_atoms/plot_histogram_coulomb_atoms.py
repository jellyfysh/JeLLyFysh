...
def axes_setting(axes: plt.Axes) -> None:
    ...
    axes : matplotlib.pyplot.Axes
        The axes object.
    """
    axes.tick_params(labelsize=plotting_functions.default['fontsize'], width=plotting_functions.default['borderwidth'],
                     pad=1, length=2)
...
def main() -> None:
    ...
    reference_plot += plotting_functions.plot_curve(axes, 'ReferenceDataCoulombAtoms.dat', r"reversible MC",
                                                    mask=value_range, color='k', linestyle='-', **line_properties)
    ...
    plotting_functions.add_legend(axes, reference_plot, bbox_to_anchor=(1.04, 1.0), loc='upper left',
                                  title="reference", handlelength=2.5)
    plotting_functions.add_legend(axes, data_plots, bbox_to_anchor=(1.04, 0.67), loc='upper left',
