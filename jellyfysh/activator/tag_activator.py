...
from jellyfysh.base.exceptions import ConfigurationError, TagActivatorError
from jellyfysh.base.logging import log_init_arguments
from jellyfysh.event_handler import EventHandler
from jellyfysh.event_handler.abstracts import StartOfRunEventHandler
...
class TagActivator(Activator):
    ...
    Each tagger provides a tag, a list of event handlers, and a method that generates the global state identifiers of
    the in-states that are required as arguments of the send_event_time method of its event handlers. A tagger further
    provides lists of tags, whose taggers will be asked for event handlers to run (creates property) or trash (trashes
    property) after an event of an event handler out of this tagger has been committed to the global state. Likewise,
    a tagger can also activate and deactivate other taggers via the activates and deactivates properties. If a tagger is
    deactivated, it will generate no in-state identifiers.

    The precise format of the global state identifiers is specified by the used state handler. They are therefore of
    ...
    candidate event of them is stored in the scheduler) and 'not running'. This allows to efficiently trash events with
    a certain tag, and to get event handlers which can compute events with a given tag.
    ...
    def __init__(self, taggers: Sequence[Tagger], internal_states: Sequence[InternalState] = ()) -> None:
        ...
        handlers are constructed in the initialize method of this class.
        ...
        super().__init__(internal_states)
        ...
        # These attributes are set in the initialize method.
        self._event_handlers = None
        self._event_handler_tagger_dictionary = None
        self._running_event_handlers = None
        self._not_running_event_handlers = None
    ...
    def initialize(self, extracted_global_state: Any) -> None:
        ...
        self._event_handlers = sum((tagger.get_event_handlers() for tagger in self._taggers), [])
        ...
        self._not_running_event_handlers = {tagger: copy.copy(tagger.get_event_handlers()) for tagger in self._taggers}
