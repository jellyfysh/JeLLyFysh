...
#
"""Module for the ExcludedCellsTagger class."""
import logging
...
class ExcludedCellsTagger(TaggerWithInternalState):
    """
    Tagger which generates pairwise in-states consisting of the active unit identifiers in the cell-occupancy system
    and all identifiers in excluded nearby cells.
    ...
    def yield_identifiers_send_event_time(
        ...
        """
        Generate pairwise in-state identifiers for the send_event_time method of this tagger's event handlers.
        ...
        The generated in-states are just tuples of length two that contain the active identifiers in the cell-occupancy
        system together with every stored identifier in the excluded cells.
        ...
        for active_cell, active_identifier in self._internal_state.yield_active_cells():
            yield from ((active_identifier, occupant_identifier)
                        for nearby_cell in self._internal_state.cells.nearby_cells(active_cell)
                        for occupant_identifier in self._internal_state[nearby_cell])
