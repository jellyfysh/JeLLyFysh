...
class ActiveRootUnitInStateTagger(Tagger):
    """
    Tagger for event handlers which require the active units corresponding to the root nodes as the in-state.

    An example for such an event handler is the RootLeafUnitActiveSwitcher.
    ...
    def __init__(self, create: Sequence[str], trash: Sequence[str], event_handler: EventHandler,
        ...
        if not setting.number_of_node_levels > 1:
            raise ConfigurationError("The tagger {0} can only be used when composite point objects are involved in the"
