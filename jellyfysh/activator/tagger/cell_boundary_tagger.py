...
class CellBoundaryTagger(TaggerWithInternalState):
    """
    Tagger which generates in-states for instances of the CellBoundaryEventHandler class.
    ...
    system will be called by the tag activator exactly when the cell of the active unit changes. By this, the internal
    state in the activator is always consistent with the global state.
    ...
    def __init__(self, create: Sequence[str], trash: Sequence[str], event_handler: CellBoundaryEventHandler,
        ...
        The event handler should be an instance of the CellBoundaryEventHandler class. The number of event handlers
        should be set to the number of active units that are relevant to the cell-occupancy system.
        ...
        if not isinstance(event_handler, CellBoundaryEventHandler):
            raise ConfigurationError("The class {0} can only be used with an instance of the class "
                                     "CellBoundaryEventHandler as the event handler.".format(self.__class__.__name__))
    ...
    def initialize_with_internal_states(self, internal_states: Sequence[InternalState]) -> None:
        ...
        This method checks if the internal state of this tagger is a cell-occupancy system. Also, the cell boundary
        event handler in the self._event_handler_to_copy attribute gets knowledge about the cell system and the cell
        level of the cell-occupancy system. This event handler is deepcopied in the subsequent call of the initialize
        method to create the desired number of (initialized) event handlers for this tagger (see Tagger base class).
    ...
    def yield_identifiers_send_event_time(
        ...
        The generated in-states for the CellBoundaryEventHandlers are just the stored active unit identifiers of the
