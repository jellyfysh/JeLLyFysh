...
from event_handler.abstracts import CellBoundingPotentialEventHandler
...
class CellBoundingPotentialTagger(TaggerWithInternalState):
    """
    Tagger which generates in-states for CellBoundingPotentialEventHandlers.
    ...
    def __init__(self, create: Sequence[str], trash: Sequence[str], event_handler: EventHandler,
        ...
        Finally the event handler should be an instance of both a CellBoundingPotentialEventHandler and an EventHandler.
        ...
        if not (isinstance(event_handler, CellBoundingPotentialEventHandler)
            ...
            raise ConfigurationError("The class {0} can only be used "
                                     "with a class inheriting from the classes CellBoundingAbstract and EventHandler!"
    ...
    def initialize(self, extracted_global_state: Sequence[Node], internal_states: Sequence[InternalState]) -> None:
        ...
        Also, the event handlers get knowledge about the cell system and the full extracted global state.
        ...
        if not isinstance(self._internal_state, CellOccupancy):
            raise ConfigurationError("Class ExcludedCellsTagger can only be used with an instance of CellOccupancy "
                                     "as the internal state!")
        self._event_handlers[0].initialize(self._internal_state.cells, extracted_global_state)
        initialized_potential = self._event_handlers[0].bounding_potential
        for index, event_handler in enumerate(self._event_handlers):
            if index == 0:
                continue
            event_handler.initialize_with_initialized_potential(self._internal_state.cells,
                                                                deepcopy(initialized_potential))
    ...
    def yield_identifiers_send_event_time(
        ...
        stored identifier in the cell-occupancy system which are not in the excluded cells.
        ...
        for active_cell, active_identifier in self._internal_state.yield_active_cells():
            yield from ((active_identifier, self._internal_state[cell])
                        for cell in self._internal_state.cells.yield_cells()
                        if self._internal_state[cell] is not None
                        and cell not in self._internal_state.cells.excluded_cells(active_cell))
