...
class TestPowerBounded(unittest.TestCase):
    def setUp(self) -> None:
        self._config = ConfigParser()
        self._ini_file = "config_files/2018_JCP_149_064113/coulomb_atoms/power_bounded.ini"
        ...
        # Replace output file
        self._config.set("SeparationOutputHandler", "filename", "TestPowerBounded.dat")

    def tearDown(self) -> None:
        # Use class method to make sure files are deleted even when a test fails
        if "TestPowerBounded.dat" in os.listdir("."):
            os.remove("TestPowerBounded.dat")
        if "TestPowerBounded.dat.tmp" in os.listdir("."):
            os.remove("TestPowerBounded.dat.tmp")
        # Reset setting
        setting.reset()
    ...
    def test_run_power_bounded(self, read_config_mock):
        argv = [self._ini_file]
        ...
        self.assertIn("TestPowerBounded.dat", os.listdir("."))
