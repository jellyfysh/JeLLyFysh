...
class TestCoulombCellVetoLjInverted(unittest.TestCase):
    def setUp(self) -> None:
        self._config = ConfigParser()
        self._ini_file = "config_files/2018_JCP_149_064113/water/coulomb_cell_veto_lj_inverted.ini"
        ...
        # Replace output file
        self._config.set("OxygenOxygenSeparationOutputHandler", "filename", "Test_CoulombCellVeto_LjInverted.dat")

    def tearDown(self) -> None:
        # Use class method to make sure files are deleted even when a test fails
        if "Test_CoulombCellVeto_LjInverted.dat" in os.listdir("."):
            os.remove("Test_CoulombCellVeto_LjInverted.dat")
        if "Test_CoulombCellVeto_LjInverted.dat.tmp" in os.listdir("."):
            os.remove("Test_CoulombCellVeto_LjInverted.dat.tmp")
        # Reset setting
        setting.reset()
