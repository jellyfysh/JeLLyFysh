...
class TestCoulombPowerBoundedLjCellBounded(unittest.TestCase):
    def setUp(self) -> None:
        self._config = ConfigParser()
        self._ini_file = "config_files/2018_JCP_149_064113/water/coulomb_power_bounded_lj_cell_bounded.ini"
        ...
        self._config.set("OxygenOxygenSeparationOutputHandler", "filename",
                         "Test_CoulombPowerBounded_LjCellBounded.dat")

    def tearDown(self) -> None:
        # Use class method to make sure files are deleted even when a test fails
        if "Test_CoulombPowerBounded_LjCellBounded.dat" in os.listdir("."):
            os.remove("Test_CoulombPowerBounded_LjCellBounded.dat")
        if "Test_CoulombPowerBounded_LjCellBounded.dat.tmp" in os.listdir("."):
            os.remove("Test_CoulombPowerBounded_LjCellBounded.dat.tmp")
        # Reset setting
        setting.reset()
    ...
    def test_run_coulomb_power_bounded_lj_cell_bounded(self, read_config_mock):
        argv = [self._ini_file]
        ...
        self.assertIn("Test_CoulombPowerBounded_LjCellBounded.dat", os.listdir("."))
