...
class TestCuboidCells(ExpandedTestCase, TestCase):
    ...
    def setUpTwoDimensionalCells(self):
        ...
        self._cells = CuboidPeriodicCells(cells_per_side=[4, 5], neighbor_layers=2)
    ...
    def test_two_dimensional_nearby_cells(self):
        self.setUpTwoDimensionalCells()
        # With the given number of neighbor layers, all cells are nearby in the periodic cell system.
        for cell in self._cells.yield_cells():
            nearby_cells = self._cells.nearby_cells(cell)
            self.assertEqual(len(nearby_cells), len(self._all_cell_identifiers))
            self.assertEqual(set(nearby_cell.identifier for nearby_cell in nearby_cells),
                             set(self._all_cell_identifiers))
    ...
    def test_two_dimensional_nearby_cells_zero_neighbor_layers(self):
        ...
        setting.set_number_of_root_nodes(1)
        setting.set_number_of_nodes_per_root_node(1)
        self._cells = CuboidPeriodicCells(cells_per_side=[4, 5], neighbor_layers=0)
    ...
    def test_two_dimensional_neighbor_cell(self):
        ...
        for cell in self._cells.yield_cells():
            if cell.identifier == (0, 0):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 0, False).identifier, (3, 0))
                self.assertEqual(self._cells.neighbor_cell(cell, 1, True).identifier, (0, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 1, False).identifier, (0, 4))
                continue
            if cell.identifier == (1, 0):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 1, False).identifier, (1, 4))
                continue
            if cell.identifier == (2, 0):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 1, False).identifier, (2, 4))
                continue
            if cell.identifier == (3, 0):
                self.assertEqual(self._cells.neighbor_cell(cell, 0, True).identifier, (0, 0))
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 1, False).identifier, (3, 4))
                continue

            if cell.identifier == (0, 1):
                self.assertEqual(self._cells.neighbor_cell(cell, 0, True).identifier, (1, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 0, False).identifier, (3, 1))
            ...
            if cell.identifier == (3, 1):
                self.assertEqual(self._cells.neighbor_cell(cell, 0, True).identifier, (0, 1))
            ...
            if cell.identifier == (0, 2):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 0, False).identifier, (3, 2))
            ...
            if cell.identifier == (3, 2):
                self.assertEqual(self._cells.neighbor_cell(cell, 0, True).identifier, (0, 2))
            ...
            if cell.identifier == (0, 3):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 0, False).identifier, (3, 3))
            ...
            if cell.identifier == (3, 3):
                self.assertEqual(self._cells.neighbor_cell(cell, 0, True).identifier, (0, 3))
            ...
            if cell.identifier == (0, 4):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 0, False).identifier, (3, 4))
                self.assertEqual(self._cells.neighbor_cell(cell, 1, True).identifier, (0, 0))
            ...
            if cell.identifier == (1, 4):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 1, True).identifier, (1, 0))
            ...
            if cell.identifier == (2, 4):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 1, True).identifier, (2, 0))
                self.assertEqual(self._cells.neighbor_cell(cell, 1, False).identifier, (2, 3))
                continue
            if cell.identifier == (3, 4):
                self.assertEqual(self._cells.neighbor_cell(cell, 0, True).identifier, (0, 4))
                self.assertEqual(self._cells.neighbor_cell(cell, 0, False).identifier, (2, 4))
                self.assertEqual(self._cells.neighbor_cell(cell, 1, True).identifier, (3, 0))
    ...
    def test_two_dimensional_zero_cell(self):
        self.setUpTwoDimensionalCells()
        self.assertEqual(self._cells.zero_cell.identifier, (0, 0))

    def test_two_dimensional_relative_cell(self):
        self.setUpTwoDimensionalCells()

        cell_one = self._cells.position_to_cell([1.0e-13, 1.0e-13])
        self.assertEqual(cell_one.identifier, (0, 0))
        cell_two = self._cells.position_to_cell([0.25 + 1.0e-13, 0.4 - 1.0e-13])
        self.assertEqual(cell_two.identifier, (1, 0))
        self.assertEqual(self._cells.relative_cell(cell_two, cell_one).identifier, (1, 0))
        self.assertEqual(self._cells.relative_cell(cell_one, cell_two).identifier, (3, 0))

        cell_one = self._cells.position_to_cell([1.0e-13, 1.0e-13])
        self.assertEqual(cell_one.identifier, (0, 0))
        cell_two = self._cells.position_to_cell([0.69, 1.45])
        self.assertEqual(cell_two.identifier, (2, 3))
        self.assertEqual(self._cells.relative_cell(cell_two, cell_one).identifier, (2, 3))
        self.assertEqual(self._cells.relative_cell(cell_one, cell_two).identifier, (2, 2))

        cell_one = self._cells.position_to_cell([1.0 - 1.0e-13, 0.6])
        self.assertEqual(cell_one.identifier, (3, 1))
        cell_two = self._cells.position_to_cell([0.48, 2.0 - 1.0e-13])
        self.assertEqual(cell_two.identifier, (1, 4))
        self.assertEqual(self._cells.relative_cell(cell_two, cell_one).identifier, (2, 3))
        self.assertEqual(self._cells.relative_cell(cell_one, cell_two).identifier, (2, 2))

        cell_one = self._cells.position_to_cell([0.5 + 1.0e-13, 0.2])
        self.assertEqual(cell_one.identifier, (2, 0))
        cell_two = self._cells.position_to_cell([0.27, 1.2 - 1.0e-13])
        self.assertEqual(cell_two.identifier, (1, 2))
        self.assertEqual(self._cells.relative_cell(cell_two, cell_one).identifier, (3, 2))
        self.assertEqual(self._cells.relative_cell(cell_one, cell_two).identifier, (1, 3))

    def test_two_dimensional_translate(self):
        self.setUpTwoDimensionalCells()
        ...
        cell_two = self._cells.position_to_cell([0.25 + 1.0e-13, 0.4 - 1.0e-13])
        self.assertEqual(cell_two.identifier, (1, 0))
        self.assertEqual(self._cells.translate(cell_two, cell_one).identifier, (1, 0))
        self.assertEqual(self._cells.translate(cell_one, cell_two).identifier, (1, 0))

        cell_one = self._cells.position_to_cell([1.0e-13, 1.0e-13])
        self.assertEqual(cell_one.identifier, (0, 0))
        cell_two = self._cells.position_to_cell([0.69, 1.45])
        self.assertEqual(cell_two.identifier, (2, 3))
        self.assertEqual(self._cells.translate(cell_two, cell_one).identifier, (2, 3))
        self.assertEqual(self._cells.translate(cell_one, cell_two).identifier, (2, 3))

        cell_one = self._cells.position_to_cell([1.0 - 1.0e-13, 0.6])
        self.assertEqual(cell_one.identifier, (3, 1))
        cell_two = self._cells.position_to_cell([0.48, 2.0 - 1.0e-13])
        self.assertEqual(cell_two.identifier, (1, 4))
        self.assertEqual(self._cells.translate(cell_two, cell_one).identifier, (0, 0))
        self.assertEqual(self._cells.translate(cell_one, cell_two).identifier, (0, 0))

        cell_one = self._cells.position_to_cell([0.5 + 1.0e-13, 0.2])
        self.assertEqual(cell_one.identifier, (2, 0))
        cell_two = self._cells.position_to_cell([0.27, 1.2 - 1.0e-13])
        self.assertEqual(cell_two.identifier, (1, 2))
        self.assertEqual(self._cells.translate(cell_two, cell_one).identifier, (3, 2))
        self.assertEqual(self._cells.translate(cell_one, cell_two).identifier, (3, 2))
    ...
    def setUpThreeDimensionalCells(self):
        ...
        setting.set_number_of_root_nodes(1)
        setting.set_number_of_nodes_per_root_node(1)
        self._cells = CuboidPeriodicCells(cells_per_side=[4, 2, 2], neighbor_layers=1)
    ...
    def test_three_dimensional_nearby_cells(self):
        ...
        for cell in self._cells.yield_cells():
            if cell.identifier == (0, 0, 0):
                expected_nearby_cells = {(3, 0, 0), (0, 0, 0), (1, 0, 0), (3, 1, 0), (0, 1, 0), (1, 1, 0),
                                         (3, 0, 1), (0, 0, 1), (1, 0, 1), (3, 1, 1), (0, 1, 1), (1, 1, 1)}
                nearby_cells = self._cells.nearby_cells(cell)
            ...
            if cell.identifier == (3, 0, 0):
                expected_nearby_cells = {(2, 0, 0), (3, 0, 0), (0, 0, 0), (2, 1, 0), (3, 1, 0), (0, 1, 0),
                                         (2, 0, 1), (3, 0, 1), (0, 0, 1), (2, 1, 1), (3, 1, 1), (0, 1, 1)}
                nearby_cells = self._cells.nearby_cells(cell)
                self.assertEqual(len(nearby_cells), len(expected_nearby_cells))
                self.assertEqual(set(nearby_cell.identifier for nearby_cell in nearby_cells), expected_nearby_cells)
                continue
    ...
    def test_three_dimensional_nearby_cells_zero_neighbor_layers(self):
        ...
        setting.set_number_of_root_nodes(1)
        setting.set_number_of_nodes_per_root_node(1)
        self._cells = CuboidPeriodicCells(cells_per_side=[4, 2, 2], neighbor_layers=0)
    ...
    def test_three_dimensional_neighbor_cell(self):
        ...
        for cell in self._cells.yield_cells():
            if cell.identifier == (0, 0, 0):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 0, False).identifier, (3, 0, 0))
                self.assertEqual(self._cells.neighbor_cell(cell, 1, True).identifier, (0, 1, 0))
                self.assertEqual(self._cells.neighbor_cell(cell, 1, False).identifier, (0, 1, 0))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, True).identifier, (0, 0, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, False).identifier, (0, 0, 1))
                continue
            if cell.identifier == (1, 0, 0):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 1, False).identifier, (1, 1, 0))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, True).identifier, (1, 0, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, False).identifier, (1, 0, 1))
                continue
            if cell.identifier == (2, 0, 0):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 1, False).identifier, (2, 1, 0))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, True).identifier, (2, 0, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, False).identifier, (2, 0, 1))
                continue
            if cell.identifier == (3, 0, 0):
                self.assertEqual(self._cells.neighbor_cell(cell, 0, True).identifier, (0, 0, 0))
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 1, False).identifier, (3, 1, 0))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, True).identifier, (3, 0, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, False).identifier, (3, 0, 1))
                continue

            if cell.identifier == (0, 1, 0):
                self.assertEqual(self._cells.neighbor_cell(cell, 0, True).identifier, (1, 1, 0))
                self.assertEqual(self._cells.neighbor_cell(cell, 0, False).identifier, (3, 1, 0))
                self.assertEqual(self._cells.neighbor_cell(cell, 1, True).identifier, (0, 0, 0))
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 2, False).identifier, (0, 1, 1))
                continue
            if cell.identifier == (1, 1, 0):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 1, True).identifier, (1, 0, 0))
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 2, False).identifier, (1, 1, 1))
                continue
            if cell.identifier == (2, 1, 0):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 1, True).identifier, (2, 0, 0))
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 2, False).identifier, (2, 1, 1))
                continue
            if cell.identifier == (3, 1, 0):
                self.assertEqual(self._cells.neighbor_cell(cell, 0, True).identifier, (0, 1, 0))
                self.assertEqual(self._cells.neighbor_cell(cell, 0, False).identifier, (2, 1, 0))
                self.assertEqual(self._cells.neighbor_cell(cell, 1, True).identifier, (3, 0, 0))
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 2, False).identifier, (3, 1, 1))
                continue

            if cell.identifier == (0, 0, 1):
                self.assertEqual(self._cells.neighbor_cell(cell, 0, True).identifier, (1, 0, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 0, False).identifier, (3, 0, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 1, True).identifier, (0, 1, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 1, False).identifier, (0, 1, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, True).identifier, (0, 0, 0))
            ...
            if cell.identifier == (1, 0, 1):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 1, False).identifier, (1, 1, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, True).identifier, (1, 0, 0))
            ...
            if cell.identifier == (2, 0, 1):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 1, False).identifier, (2, 1, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, True).identifier, (2, 0, 0))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, False).identifier, (2, 0, 0))
                continue
            if cell.identifier == (3, 0, 1):
                self.assertEqual(self._cells.neighbor_cell(cell, 0, True).identifier, (0, 0, 1))
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 1, False).identifier, (3, 1, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, True).identifier, (3, 0, 0))
            ...
            if cell.identifier == (0, 1, 1):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 0, False).identifier, (3, 1, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 1, True).identifier, (0, 0, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 1, False).identifier, (0, 0, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, True).identifier, (0, 1, 0))
            ...
            if cell.identifier == (1, 1, 1):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 1, True).identifier, (1, 0, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 1, False).identifier, (1, 0, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, True).identifier, (1, 1, 0))
            ...
            if cell.identifier == (2, 1, 1):
                ...
                self.assertEqual(self._cells.neighbor_cell(cell, 1, True).identifier, (2, 0, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 1, False).identifier, (2, 0, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, True).identifier, (2, 1, 0))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, False).identifier, (2, 1, 0))
                continue
            if cell.identifier == (3, 1, 1):
                self.assertEqual(self._cells.neighbor_cell(cell, 0, True).identifier, (0, 1, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 0, False).identifier, (2, 1, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 1, True).identifier, (3, 0, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 1, False).identifier, (3, 0, 1))
                self.assertEqual(self._cells.neighbor_cell(cell, 2, True).identifier, (3, 1, 0))
    ...
    def test_three_dimensional_zero_cell(self):
        self.setUpThreeDimensionalCells()
        self.assertEqual(self._cells.zero_cell.identifier, (0, 0, 0))

    def test_three_dimensional_relative_cell(self):
        self.setUpThreeDimensionalCells()

        cell_one = self._cells.position_to_cell([1.0e-13, 1.0e-13, 1.0e-13])
        self.assertEqual(cell_one.identifier, (0, 0, 0))
        cell_two = self._cells.position_to_cell([0.33, 0.38, 1.0 - 1.0e-13])
        self.assertEqual(cell_two.identifier, (1, 0, 1))
        self.assertEqual(self._cells.relative_cell(cell_two, cell_one).identifier, (1, 0, 1))
        self.assertEqual(self._cells.relative_cell(cell_one, cell_two).identifier, (3, 0, 1))

        cell_one = self._cells.position_to_cell([1.0e-13, 1.0e-13, 1.0e-13])
        self.assertEqual(cell_one.identifier, (0, 0, 0))
        cell_two = self._cells.position_to_cell([0.75 - 1.0e-13, 0.82, 0.37])
        self.assertEqual(cell_two.identifier, (2, 1, 0))
        self.assertEqual(self._cells.relative_cell(cell_two, cell_one).identifier, (2, 1, 0))
        self.assertEqual(self._cells.relative_cell(cell_one, cell_two).identifier, (2, 1, 0))

        cell_one = self._cells.position_to_cell([0.44, 0.55, 0.73])
        self.assertEqual(cell_one.identifier, (1, 1, 1))
        cell_two = self._cells.position_to_cell([1.0 - 1.0e-13, 0.90, 0.09])
        self.assertEqual(cell_two.identifier, (3, 1, 0))
        self.assertEqual(self._cells.relative_cell(cell_two, cell_one).identifier, (2, 0, 1))
        self.assertEqual(self._cells.relative_cell(cell_one, cell_two).identifier, (2, 0, 1))

        cell_one = self._cells.position_to_cell([0.25 - 1.0e-13, 0.5 + 1.0e-13, 0.17])
        self.assertEqual(cell_one.identifier, (0, 1, 0))
        cell_two = self._cells.position_to_cell([0.81, 0.49, 0.52])
        self.assertEqual(cell_two.identifier, (3, 0, 1))
        self.assertEqual(self._cells.relative_cell(cell_two, cell_one).identifier, (3, 1, 1))
        self.assertEqual(self._cells.relative_cell(cell_one, cell_two).identifier, (1, 1, 1))

    def test_three_dimensional_translate(self):
        self.setUpThreeDimensionalCells()
        ...
        cell_two = self._cells.position_to_cell([0.33, 0.38, 1.0 - 1.0e-13])
        self.assertEqual(cell_two.identifier, (1, 0, 1))
        self.assertEqual(self._cells.translate(cell_two, cell_one).identifier, (1, 0, 1))
        self.assertEqual(self._cells.translate(cell_one, cell_two).identifier, (1, 0, 1))

        cell_one = self._cells.position_to_cell([1.0e-13, 1.0e-13, 1.0e-13])
        self.assertEqual(cell_one.identifier, (0, 0, 0))
        cell_two = self._cells.position_to_cell([0.75 - 1.0e-13, 0.82, 0.37])
        self.assertEqual(cell_two.identifier, (2, 1, 0))
        self.assertEqual(self._cells.translate(cell_two, cell_one).identifier, (2, 1, 0))
        self.assertEqual(self._cells.translate(cell_one, cell_two).identifier, (2, 1, 0))

        cell_one = self._cells.position_to_cell([0.44, 0.55, 0.73])
        self.assertEqual(cell_one.identifier, (1, 1, 1))
        cell_two = self._cells.position_to_cell([1.0 - 1.0e-13, 0.90, 0.09])
        self.assertEqual(cell_two.identifier, (3, 1, 0))
        self.assertEqual(self._cells.translate(cell_two, cell_one).identifier, (0, 0, 1))
        self.assertEqual(self._cells.translate(cell_one, cell_two).identifier, (0, 0, 1))

        cell_one = self._cells.position_to_cell([0.25 - 1.0e-13, 0.5 + 1.0e-13, 0.17])
        self.assertEqual(cell_one.identifier, (0, 1, 0))
        cell_two = self._cells.position_to_cell([0.81, 0.49, 0.52])
        self.assertEqual(cell_two.identifier, (3, 0, 1))
        self.assertEqual(self._cells.translate(cell_two, cell_one).identifier, (3, 1, 1))
        self.assertEqual(self._cells.translate(cell_one, cell_two).identifier, (3, 1, 1))

    def test_cuboid_periodic_cells_raises_error_if_hypercuboid_setting_is_not_initialized(self):
        with self.assertRaises(ConfigurationError):
            CuboidPeriodicCells(cells_per_side=[4, 2, 2], neighbor_layers=1)

    def test_cuboid_periodic_cells_raises_error_if_empty_cells_per_side_is_given(self):
        hypercubic_setting.HypercubicSetting(beta=1.0, dimension=3, system_length=1.0)
        # Set these values so that the setting package is fully initialized.
        setting.set_number_of_node_levels(1)
        setting.set_number_of_root_nodes(1)
        setting.set_number_of_nodes_per_root_node(1)
        with self.assertRaises(ConfigurationError):
            self._cells = CuboidPeriodicCells(cells_per_side=[], neighbor_layers=1)

    def test_cuboid_periodic_cells_raises_error_if_too_many_cells_per_side_are_given(self):
        hypercubic_setting.HypercubicSetting(beta=1.0, dimension=3, system_length=1.0)
        # Set these values so that the setting package is fully initialized.
        setting.set_number_of_node_levels(1)
        setting.set_number_of_root_nodes(1)
        setting.set_number_of_nodes_per_root_node(1)
        with self.assertRaises(ConfigurationError):
            self._cells = CuboidPeriodicCells(cells_per_side=[3, 3, 3, 3], neighbor_layers=1)

    def test_cuboid_periodic_cells_raises_error_if_negative_neighbor_layers_is_given(self):
        hypercubic_setting.HypercubicSetting(beta=1.0, dimension=3, system_length=1.0)
        # Set these values so that the setting package is fully initialized.
        setting.set_number_of_node_levels(1)
        setting.set_number_of_root_nodes(1)
